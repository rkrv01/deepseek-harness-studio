...
桌面 Demo 负责监管现有的回环 Web Host；窗口关闭后，系统托盘继续持有 Host 的生命周期。

## 开发

安装依赖后，使用开发命令。首次运行、相关输入发生变化或关键产物缺失时，该命令会先构建 Host 与客户端包和 Web 前端，再在默认浏览器中启动 Web Host（不启动 Electron 壳）：

```sh
pnpm run dev:desktop
```

需要 Electron 壳（托盘驱动的 Host 生命周期、插件中心、Preset 广场与应用中心）时运行：
...
在 Electron 壳中，关闭窗口会隐藏窗口。通过托盘菜单恢复窗口或退出应用。显式退出会等待 Host 进程停止，并在 Host 的有界宽限期结束后升级终止行为。
...
完整的 Demo 打包、验收和交付手册见 [PACKAGING.zh.md](PACKAGING.zh.md)。
...
打包后的应用通过 Electron 的 Node 模式，在独立进程内运行已暂存的 `@deepseek-ai/dsh` CLI。应用因此保留受 supervisor 管理的 Host 生命周期，无需携带第二个 Node 可执行文件。如果暂存的 CLI 入口或 Web 前端入口缺失，`afterPack` 检查会在签名前拒绝该产物；它还会核验 Harness 图片管线所需的 macOS arm64 或 Windows x64 Sharp 原生模块。Starlight Harness 演示版暂不启用在线更新，因此未配置通用 HTTPS 更新源时不会写入 `app-update.yml`。macOS 和 Windows 都从受跟踪、带透明圆角的 `apps/desktop/build/icon.png` 派生平台图标；仓库不提交独立的平台专用变体。

### macOS 演示 DMG 与 ZIP

macOS 发布命令会生成用于安装的 DMG 和 ZIP 归档。未提供签名和公证凭据时，它会构建一个未签名的临时演示产物：

```sh
pnpm run dist:mac:desktop
```

未签名演示包分发到其他 Mac 时可能触发系统安全提示。正式发布时，构建用户的 Keychain 中需要安装有效的 `Developer ID Application` 身份，且证书与私钥必须同时存在；同时还需要一组完整的公证凭据。Keychain profile 可以避免应用专用密码进入仓库或 shell 历史记录：
...
完成签名构建后，挂载生成的 DMG，再验证其中应用的签名、Gatekeeper 评估和已装订的公证票据：
...
Starlight Harness 演示版不启用在线更新。设置页会显示在线更新暂未开放，演示打包也不要求通用更新源。后续正式发布可以配置独立的 Starlight 更新源，并在安装包通过平台签名与验收检查后发布一个或多个平台产物目录：
...
替换或移除现有安装前，NSIS 会请求正在运行的单实例进入普通显式退出路径，最多等待五秒让 supervisor 管理的 Host 停稳，再以两次有界尝试终止任何残留的 `Starlight Harness.exe` 进程树。如果仍有进程占用安装目录，操作会明确失败，不会留下半卸载状态。如果手动删除或卸载失败留下损坏的注册信息和不完整的专属 `Starlight Harness` 应用目录，替换程序会自动清理残留、跳过不可用的旧卸载器、写入干净载荷并重建卸载注册信息。Profile 数据位于应用目录之外，不受该替换影响。
...
macOS 和 Windows x64 已有演示发布路径。当前 Starlight Harness 演示版不启用在线更新、macOS 公证或 Windows Authenticode 签名。Linux 目前仍只生成未封装应用，尚无安装包格式与发行签名路径。
