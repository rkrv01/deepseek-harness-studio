...
对 `kind: 'document'`，`ComposerAttachments` 在图片栏上方渲染紧凑芯片。每个芯片显示根据文件名推导的格式徽标，省略过长名称，并通过 `onRemoveDocument` 移除运行时注册；这里没有预览、历史渲染器或字节检查。拖放与粘贴路由仍由持有方通过 `onAddImages` 决定，后者可以把混合的项目智脑批次转发给 conversation 包的文档入口。
...
- **演示文档只有输入框芯片** — 项目智脑文档在草稿阶段暴露名称、格式徽标和移除操作；由于文件字节既不上传也不持久化，没有持久历史渲染器。
