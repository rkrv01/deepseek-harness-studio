...
For `kind: 'document'`, `ComposerAttachments` renders compact chips above the image rail. Each chip shows a format badge derived from the filename, ellides long names, and removes the runtime registration through `onRemoveDocument`; there is no preview, history renderer, or byte inspection. Drop and paste routing stays with the owner through `onAddImages`, which may forward a mixed Project Brain batch to the conversation package's document intake.
...
- **Demo documents have composer chips only** — Project Brain documents expose name, type badge, and removal in the draft; they have no durable history renderer because their bytes are neither uploaded nor persisted.
