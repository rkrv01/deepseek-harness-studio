...
import { setProductEntryVisibility } from './entry-visibility.ts'
...
export function apply(ctx: ClientContext): void {
  ...
  let visibilityScope: ReturnType<NonNullable<typeof ctx.settingsScope>['bind']> | undefined
  try {
    const settingsScope = ctx.get('settingsScope')
    visibilityScope = settingsScope?.bind({
      namespace: 'project-brain',
      decode: (value: unknown) => typeof value === 'object' && value !== null && !Array.isArray(value) ? value as Record<string, unknown> : undefined,
    })
  } catch {
    visibilityScope = undefined
  }
  const syncVisibility = (closeHiddenPage: boolean): void => {
    const value = visibilityScope?.getSnapshot().value as {
    ...
    setProductEntryVisibility({
      showPluginCenter: value?.showPluginCenter === true,
      showPluginDiscovery: value?.showPluginDiscovery === true,
      showPresetSquare: value?.showPresetSquare === true,
      showAppCenter: value?.showAppCenter === true,
    })
    if (!closeHiddenPage) return
    if (value?.showPluginCenter !== true) ctx.layout.closePrimaryPage(PLUGIN_CENTER_PAGE_ID)
    if (value?.showPluginDiscovery !== true) ctx.layout.closePrimaryPage(PLUGIN_DISCOVERY_PAGE_ID)
    if (value?.showPresetSquare !== true) ctx.layout.closePrimaryPage(PRESET_SQUARE_PAGE_ID)
    if (value?.showAppCenter !== true) ctx.layout.closePrimaryPage(APPLICATION_CENTER_PAGE_ID)
  }
  syncVisibility(false)
  visibilityScope?.subscribe(() => { syncVisibility(true) })
  ...
  const presetInjected = (): PresetSquarePageInjected => ({
    ...
    listLocalPresets: async () => {
      ...
      return { presets: response.result.value.presets, authorable: response.result.value.authorable }
    },
    ...
    useLocalPreset: async (id) => {
      ...
      const currentWorkspace = currentSession === undefined ? undefined : workspaces.items.find(item => item.sessionIds.includes(currentSession))?.workspaceId
  ...
  const applicationInjected = (): ApplicationCenterInjected => ({
    inspectLlmWiki: async (): Promise<ApplicationRuntimeStatus> => {
      ...
      return { available: value.installed === true, credentialConfigured: value.credentialConfigured === true }
    },
    openLlmWiki: () => { const opened = window.open(LLM_WIKI_OPEN_PATH, '_blank'); if (opened !== null) opened.opener = null },
    openModelSettings: () => { ctx.settingsNavigation.open({ sectionId: 'models' }) },
    getLlmWikiSidebarVisible: () => window.localStorage.getItem(LLM_WIKI_SIDEBAR_VISIBILITY_KEY) === 'true',
  ...
  ctx.slots.inject('sidebar.primary.action', () => ctx.slots.register({
    name: 'sidebar.primary.action', id: PRESET_SQUARE_PAGE_ID, order: 22, locale: NS, inject: presetNavInjected,
  }, PresetSquareNavItem))
  ctx.slots.inject('sidebar.primary.action', () => ctx.slots.register({
    name: 'sidebar.primary.action', id: APPLICATION_CENTER_PAGE_ID, order: 23, locale: NS, inject: applicationNavInjected,
  ...
  ctx.slots.inject('main.page', () => ctx.slots.register({
    name: 'main.page', key: PRESET_SQUARE_PAGE_ID, locale: NS, inject: presetInjected,
  }, PresetSquarePage))
  ctx.slots.inject('main.page', () => ctx.slots.register({
    name: 'main.page', key: APPLICATION_CENTER_PAGE_ID, locale: NS, inject: applicationInjected,
