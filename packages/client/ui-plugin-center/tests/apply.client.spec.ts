...
describe('ui-plugin-center browser plugin', () => {
  it('registers Plugin Center, Plugin Discovery, Preset Square, and Application Center as independent pages', async () => {
    ...
    expect(navs.find(entry => entry.options.id === 'preset-square')).toBeDefined()
    expect(navs.find(entry => entry.options.id === 'application-center')).toBeDefined()
    ...
    expect(pages.find(entry => entry.options.key === 'preset-square')).toBeDefined()
    expect(pages.find(entry => entry.options.key === 'application-center')).toBeDefined()
