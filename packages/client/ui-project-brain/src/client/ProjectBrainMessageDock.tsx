...
export function ProjectInitializationProgress() {
  ...
  return <section className={css.initializing} aria-live="polite" aria-label="正在初始化项目">
    <div className={css.initializingHead}><span className={css.loadingDot} /><div><strong>正在初始化项目</strong><p>正在将确认的方案同步到项目智脑…</p></div><span className={css.percent}>{Math.min(92, 22 + activeStep * 24)}%</span></div>
