...
export interface SessionInput extends InputTarget {
  ...
  /** Append ordered browser-only document ids; busy admission phases refuse. */
  addDocuments(ids: readonly DraftAttachmentId[]): boolean
  /** Remove one browser-owned image id; busy admission phases refuse. */
  removeImage(id: DraftAttachmentId): void
  /** Remove one browser-only document id; busy admission phases refuse. */
...
export interface InputState {
  ...
  /** Ordered runtime-only document ids; bytes stay in the browser and metadata alone reaches handlers. */
