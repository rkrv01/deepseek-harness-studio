...
export function apply(ctx: Context): void {
        ...
        addDocuments: (files) => {
          try {
            ...
            if (!shell.addDocuments(documents.map(document => document.id))) {
              conversation.releaseDraftDocuments(documents)
            }
            return null
          } catch (error: unknown) {
            return error instanceof Error ? error.message : String(error)
  ...
  slots.register({
    ...
    inject: (sessionId: SessionId, actions: BoundActions<typeof chatStore>): ChatViewInjected => {
      ...
      return {
        ...
        assistantSurface: payload => ctx.get('assistantSurface')?.renderLeading(payload) ?? null,
        openFile: (path) => {
  ...
  slots.register({
    ...
    children: {
      ...
      'conversation.details.workbench': { kind: 'single', scope: 'session' },
    },
    store: chatStore,
