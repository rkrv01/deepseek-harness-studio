...
export type ProjectBrainReplyKind = 'launch-plan' | 'launch-receipt' | 'platform-retry' | 'meeting-analysis' | 'meeting-receipt' | 'executive-briefing' | 'briefing-receipt' | 'handoff' | 'fallback'
...
export function resolveProjectBrainReply(prompt: string): ProjectBrainReply {
  ...
  if (envelope?.scenarioId === 'executive-briefing' && envelope.action === 'confirm') return { kind: 'briefing-receipt', text: executiveBriefingReceipt() }
...
function executiveBriefingReceipt(): string {
