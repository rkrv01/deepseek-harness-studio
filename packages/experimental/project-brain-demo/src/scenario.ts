...
import { PROJECT_BRAIN_PLAN } from '@deepseek-ai/dsh-client-ui-project-brain/src/project-data.ts'
import type { ProjectBrainPlanData } from '@deepseek-ai/dsh-client-ui-project-brain/src/project-data.ts'

export type ProjectBrainReplyKind = 'launch-plan' | 'launch-receipt' | 'platform-retry' | 'handoff' | 'fallback'

export interface ProjectBrainReply { readonly kind: ProjectBrainReplyKind; readonly text: string }
export { PROJECT_BRAIN_PLAN }
...
export function resolveProjectBrainReply(prompt: string): ProjectBrainReply {
  ...
  if (/会议纪要|开完.*会|整理一次项目会议/u.test(prompt)) return { kind: 'handoff', text: handoff('会议纪要', '把会议中的决定、责任人和截止时间整理成可执行事项。') }
